...
const listWrapper = document.querySelector('.list-btn_wrapper');
...
// Эта функция отрабатывает лишь в точках перелома
const handleQueryListener = () => {
  // Создает объект который говорит какие точки передома сейчас соотвецтвуют
  ...
  `;
    ...
    myGallery.innerHTML = createGallery(galleryItems);
  } else if (updateMatches.tablet) {
  ...
  <li class="animation-section_item">
    ...
    <div class="animation-section_img-block">
      ...
      <img class="animation-section_img" src="${original}" data-source="${preview}" data-gif="${gif}" data-description="${description}" data-name="${name}" data-link="${link}" alt="responsive image">
  ...
  `;
    ...
    function toggleItems() {
      showItemsBtn.classList.add('animation-section_btn-more--disable');
...
function showModal(e) {
  ...
  modalImg.setAttribute('src', e.target.dataset.gif);
...
function next() {
  ...
  modalImg.setAttribute('src', nextImage.dataset.gif);
...
function prev() {
  ...
  modalImg.setAttribute('src', prevImage.dataset.gif);
